...
module.exports = function (app, pool) {
  ...
  app.get("/", (req, res) => {
    const query =
      ...
      "SELECT u.username, p.id, p.title, p.loc_barrio, p.description, p.post_date, p.loc_street, p.phone FROM posts p INNER JOIN users u ON p.user_id = u.id WHERE p.status='available' order by p.post_date asc";
